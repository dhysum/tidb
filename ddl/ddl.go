...
func (dc *ddlCtx) notifyReorgJobStateChange(job *model.Job) {
...
func cancelRunningJob(sess *sess.Session, job *model.Job,
	...
	// Does not support `admin cancel ddl jobs` during Paused/Pausing
...
// CancelJobsBySystem , internal only
...
// PauseJobsBySystem , internal only
...
// ResumeJobsBySystem , internal only
...
// processAllJobs , process all Jobs from the job table, 100 jobs each time in case of high memory usage
func processAllJobs(process func(*sess.Session, *model.Job, model.AdminCommandOperator) (err error),
	...
	for {
		jobs, err := getJobsBySQL(ns, JobTable,
			fmt.Sprintf("job_id >= %s order by job_id limit %s",
...
// PauseAllJobsBySystem , internal only
...
// ResumeAllJobsBySystem , internal only
